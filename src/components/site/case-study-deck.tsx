...
import type { CaseStudySlide } from "./case-study-types";
...
export function CaseStudyDeck({ slides }: { slides: CaseStudySlide[] }) {
...
function SlidePanel({ slide, order, total }: { slide: CaseStudySlide; order: number; total: number }) {
  return (
    <section
      aria-label={`Slide ${order + 1} of ${total}: ${slide.title}`}
...
function MobileSlide({ slide, order }: { slide: CaseStudySlide; order: number }) {
  return (
    <section
